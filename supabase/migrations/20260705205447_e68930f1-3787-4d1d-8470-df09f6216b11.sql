-- 1. Enum de papéis
CREATE TYPE public.app_role AS ENUM ('vesta', 'membro');
...
CREATE POLICY "Vesta vê todos os perfis"
  ON public.profiles FOR SELECT TO authenticated
...
CREATE POLICY "Vesta edita qualquer perfil"
  ON public.profiles FOR UPDATE TO authenticated
  USING (public.is_vesta()) WITH CHECK (public.is_vesta());
...
CREATE POLICY "Vesta vê todos papéis"
  ON public.user_roles FOR SELECT TO authenticated
  USING (public.is_vesta());

-- Escrita apenas via service_role (server functions autorizadas)

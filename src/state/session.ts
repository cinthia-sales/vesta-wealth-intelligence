...
export type PersonaId = "cinthia" | "paulo";
...
export const PERSONAE: Record<PersonaId, PersonaInfo> = {
  cinthia: {
    id: "cinthia",
    name: "Cínthia",
...
export type Scope = {
  ...
  seePersonae: PersonaId[];     // outras Personae que este membro pode ver
};

export const DEFAULT_SCOPES: Record<PersonaId, Scope> = {
...
export function allowedProfiles(loggedAs: PersonaId, scope: Scope): ProfileId[] {
  ...
  if (PERSONAE[loggedAs].role === "vesta") {
    return ["cinthia", "paulo", "familiar"];
  }
  const list: ProfileId[] = [loggedAs];
